			...
			elseif ($para1 == "ngb_members") {
			 ...
			 elseif ($para1 == "add_member") {
				if ($para2 == "") {
					...
					$this->form_validation->set_rules('mobile', 'Mobile Number', 'required|is_unique[member.mobile]', array('required' => 'The %s is required.', 'is_unique' => 'This %s already exists.'));
...
public function award($para1 = '', $para2 = '')
{
    if ($this->admin_permission() == FALSE) {
        ...
        if ($para1 == 'add') {
            ...
            // Get common fields - they exist in both forms now
            $area_id   = (int)$this->input->post('area_id');
            $legion_id = (int)$this->input->post('legion_id');
            ...
            if ($award_for == 'legion') {
                ...
                $legion_json = array(
                    ...
                    'project_name'      => $this->input->post('form_project_name'),
                    'project_date'      => $this->input->post('form_project_date'),
                    'major_achievements' => $this->input->post('form_major_achievements_legion')
                );

                $data['legion_form_json'] = json_encode($legion_json);

            } else {
                // REMOVED: $data['member_id'] = (int)$this->input->post('member_id');
                ...
                $individual_json = array(
                    'proposed_by'          => $this->input->post('form_proposed_by'),
                ...
                $data['individual_form_json'] = json_encode($individual_json);
            }
            ...
            // Helper function for simple upload
            ...
            if ($award_for == 'legion') {
                ...
                // Decode, add files and criteria, re-encode
    ...
    // Decode the JSON data based on award type
    ...
    // Ensure criteria_data is an array
    if (!isset($form_data['criteria_data']) || !is_array($form_data['criteria_data'])) {
        $form_data['criteria_data'] = [];
    }
    ...
    $filename = 'Award_Details_' . $entry['id'] . '_' . date('Ymd');
    $this->pdf->create($html, $filename, true, true);
}
        ...
        elseif ($para1 == 'delete') {
            ...
            if ($entry) {
                ...
                if ($entry['award_for'] == 'legion') {
                    $legion_json = json_decode($entry['legion_form_json'], true);
                    if (isset($legion_json['support_doc']) && file_exists($legion_json['support_doc'])) {
                        unlink($legion_json['support_doc']);
                    }
                    if (isset($legion_json['criteria_data'])) {
                        foreach ($legion_json['criteria_data'] as $criteria) {
                            if (isset($criteria['images'])) {
                                foreach ($criteria['images'] as $image) {
                    ...
                    $ind_json = json_decode($entry['individual_form_json'], true);
                    if (isset($ind_json['photo']) && file_exists($ind_json['photo'])) {
                        unlink($ind_json['photo']);
                    }
                    if (isset($ind_json['support_doc']) && file_exists($ind_json['support_doc'])) {
                        unlink($ind_json['support_doc']);
                    }
                    if (isset($ind_json['criteria_data'])) {
                        foreach ($ind_json['criteria_data'] as $criteria) {
                            if (isset($criteria['images'])) {
                                foreach ($criteria['images'] as $image) {
                                    if (file_exists($image)) {
                                        unlink($image);
            ...
            if (!empty($ids) && is_array($ids)) {
                // First get all entries to delete files
                $this->db->where_in('id', $ids);
                $entries = $this->db->get('award_entries')->result_array();
