<div id="content-container">
    <div id="page-head">
        <div id="page-title">
            <h1 class="page-header text-overflow">Offline Contribution Cart</h1>
            <div class="searchbox">
                <div class="pull-right">
                    <a href="<?= base_url('offline_contribution/make') ?>" class="btn btn-default btn-sm btn-labeled">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">Contribution Cart</li>
        </ol>
    </div>
        ...
        <?php if(empty($cart_members)): ?>
            <div class="panel">
                <div class="panel-body text-center" style="padding: 60px;">
                    ...
                    <h3>Your Contribution Cart is Empty</h3>
                    <p class="text-muted">Please add members to cart from the member list.</p>
                    <a href="<?= base_url('offline_contribution/make') ?>" class="btn btn-primary btn-lg" style="margin-top: 20px;">
                        <i class="fa fa-users"></i> Go to Member List
                    </a>
                </div>
            </div>
        <?php else: ?>
            <form method="POST" action="<?= base_url('offline_contribution/offline_process_contribution') ?>" id="offline_payment_form">
                <div class="row">
                    <!-- Left: Members List -->
                    <div class="col-md-8">
                        <div class="panel">
                    ...
                    <div class="col-md-4">
                        <div class="panel panel-primary">
                            <div class="panel-heading">
                                <h3 class="panel-title">Contribution Details</h3>
                            </div>
                            <div class="panel-body">
                                ...
                                <div id="offline_amount_summary">
                                    <h4 class="mar-no pad-btm">Contribution Summary</h4>
                                ...
                                <button type="submit" class="btn btn-success btn-block btn-lg" 
                                    ...
                                    <i class="fa fa-check-circle"></i> Process Contribution
                                </button>
                            </div>
                        </div>
                    </div>
...
$(document).ready(function(){
    ...
    $('.offline_remove_btn').on('click', function(){
        ...
        if(confirm('Remove ' + member_name + ' from cart?')){
            $.ajax({
                url: '<?= base_url("offline_contribution/offline_remove_from_cart") ?>',
                type: 'POST',
    ...
    $('#offline_clear_cart_btn').on('click', function(){
        if(confirm('Are you sure you want to clear entire cart?')){
            $.ajax({
                url: '<?= base_url("offline_contribution/offline_clear_cart") ?>',
                type: 'POST',
                dataType: 'json',
                success: function(response){
                    if(response.success){
    ...
    $('#offline_payment_form').on('submit', function(e){
        ...
        if(!confirm('Process contribution for ' + $('#offline_total_members').text() + ' members?')){
            e.preventDefault();
            return false;
        }
